...
class UIController {
    ...
    setupEventListeners() {
        ...
        // Botão para remover todos os observadores
        document.getElementById('clear-observers-btn').addEventListener('click', () => {
            this.taskSubject.clearObservers(); // Limpa os observadores
            this.notifications.push({
                getHtmlRepresentation: () => `<div class="list-group-item list-group-item-danger">Todos os observadores foram removidos.</div>`
            });
            this.renderNotifications();

            // Desmarcar os checkboxes
            document.getElementById('screen-observer').checked = false;
            document.getElementById('email-observer').checked = false;
            document.getElementById('log-observer').checked = false;
});
    }
    ...
    applyDecorators() {
        ...
        // Aplicar decorador de etapa, se selecionado
        if (document.getElementById('color-label').checked) {
            const color = document.getElementById('color-select').value;
            task = new EtapaDecorator(task, color);
        }
